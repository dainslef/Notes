...
- [Shell 语法](#shell-语法)
	...
	- [数组](#数组)
	- [条件语法](#条件语法)
	- [if/switch/case (分支语法)](#ifswitchcase-分支语法)
	- [for/while (循环语法)](#forwhile-循环语法)
	- [select 语句](#select-语句)
	- [echo 函数](#echo-函数)
	- [函数定义](#函数定义)
...
fish使用`-D`或`--debug-stack-frames`参数设定脚本调试级别，按调试级别输出不同程度的调试信息：

```
$ fish -D [调试级别] [fish脚本]
$ fish --debug-stack-frames=[调试级别] [fish脚本]
```
...
${file#*/} # 获取首个"/"之后的内容: path_b/abc.cde.hs
${file##*/} # 获取最后一个"/"之后的内容: abc.cde.hs
${file#*.} # 获取首个"."之后的内容: cde.hs
${file##*.} # 获取最后一个"."之后的内容: hs
```

使用`%`/`%%`可从字符串中提取特定字符之前的内容：

```sh
file="path_a/path_b/abc.cde.hs"

${file#/*} # 获取首个"/"之前的内容: path_a
...
## 管道
Shell中的**管道**操作符`|`用于将一个进程的输出作为另一进程的输入。
...
以`ls`指令为例，假设当前路径下**不存在**`xxx`文件，
使用`ls xxx`指令时会得到错误输出，提示文件不存在：
...
## 变量
bash/zsh变量相关语法基本相同，fish与其有较大差异。
...
- `$变量名` 获取变量内容
- `set` 列出已定义的变量
- `unset 变量名` 删除指定名称的变量(bash/zsh语法)
- `set -e 变量名` 删除指定名称的变量(fish语法)

## 本地变量
`Shell`中修改变量与定义变量语法相同，需要使用额外的作用域关键字区分访问外部变量或是新定义本地变量。

bash/zsh中使用`local`关键字定义**本地变量**。
...
bash中本地变量仅能在函数中定义。
zsh扩展了`local`关键字的用法，可以在任意区域使用`local`关键字定义变量，
使用`local`定义的变量之前若存在非`local`形式的定义，则隐藏之前的定义而非改写其值。

fish中使用`set`函数定义变量，使用`set -l`定义**本地变量**。
...
- `shift` 对于数量超过9个的参数表，使用shift指令从左边移走参数，以访问更多参数
...
## 随机数
bash/zsh/fish均内置了生成随机数的功能。

- bash/zsh中通过访问`RANDOM`变量获取随机数：
...
## Fork Bomb
`Fork Bomb`(**Fork炸弹**)是bash/zsh中存在的经典恶意脚本：
...
Fork炸弹利用bash/zsh中简短的函数语法，使用符号`:`或`.`作为函数名，在函数体中递归调用自身。
函数正常的写法如下：
...
Fork炸弹在其它Shell中并不有效，如fish中函数需要使用`function/end`关键字定义，代码不具有迷惑性；而csh/tcsh中不支持函数。
