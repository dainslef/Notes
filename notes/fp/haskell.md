...
`$`函數優先級爲0，函數優先級最低，低於幾乎所有的常見函數/操作符，
使用`$`運算符將函數與之後的參數分隔開，能代替括號操作符改變操作優先級。
...
`.`函數擁有最高的優先級，常用於`Pointfree`風格中，通過函數組合來代替多個連續的函數調用。

## `&` 函數
`&`函數與OCaml家族的管道運算符`|>`相同，函數定義：

```hs
Prelude> import Data.Function (&)
Prelude Data.Function> :info (&)
(&) :: a -> (a -> b) -> b 	-- Defined in ‘Data.Function’
infixl 1 &
```

`&`函數不在默認導入中，而位於`Data.Function`模塊中，需要手動導入。
`&`函數用於將一些複雜的組合表達式轉換為鏈式操作，示例：

```hs
Prelude> foldl1 (\a b -> a + b) (map (\n -> n + 1) [1, 2, 3])
9
Prelude> import Data.Function ((&))
Prelude Data.Function> [1, 2, 3] & map (\n -> n + 1) & foldl1 (\a b -> a + b)
9
```
