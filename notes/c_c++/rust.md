...
- 現代化的構建系統
    ...
    第三方庫並未廣泛接納模塊系統，甚至標準庫在C++20中亦未使用模塊系統，而是在C++23中逐步開始使用。
...
- 現代化的標準庫設計
    ...
    Rust標準庫具備更加現代化的API設計，設計清晰，風格一致，使用簡單。
...
- 元數據與宏
    ...
    導致C++依舊使用繼承自C的文本宏作為生成代碼的方式，缺乏統一規範且不利於編譯器進行靜態分析。

    Rust支持derive機制和自定義屬性（過程宏），並提供API可由類庫解析並安全地在編譯時生成代碼，
    增強了各類社區庫的表現力與API設計。Rust提供的聲明式宏相比C/C++也更加規範和安全。
...
1. 字符串處理
    ...
    早期版本僅可使用C標準庫`sprintf()`或其它社區庫，如Boost Format。
    至今（C++23）依舊不支持字符串插值。
...
1. 類型轉換
    ...
    C++無標準類型轉換接口，各類庫提供接口不一致，或者使用存在風險的構造函數隱式轉換。
...
### 依賴配置
在Cargo.toml的`[dependencies]`配置段中設置項目依賴：
...
### features（特性）
[`features`（特性）](https://doc.rust-lang.org/cargo/reference/features.html)
功能類似C/C++中基於宏的條件編譯功能，用於提供選項開啟項目的可選功能，
在Cargo.toml中可定義項目中支持的特性：

```toml
[features]
default = [特性1, 特性2, ...] # 設置默認啟用的特性，不啟用任何特性可不設置此配置項
特性名稱1 = [依賴1, 依賴2, 依賴3/依賴3特性, ...] # 根据特性启用依赖，或者依賴的特性
特性名稱2 = [特性3, 特性4, ...] # 根据特性启用本模塊的其它特性
...

[dependencies]
依賴1 = { version = "版本", optional = true } # 需要通過特性啟用的依賴需要設置為可選依賴
...
```

定義特性後，可在代碼中編寫需要指定特性才能啟用的代碼：
...
// 當不存在XX特性時啟用代碼
#[cfg(not(feature = "XX特性"))]
... rust codes ...
```

在編譯項目時可通過`-F/--features`開啟特性：

```
$ cargo build --features 特性1,特性2,...
```
