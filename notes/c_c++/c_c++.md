...
- [C/C++中`static`關鍵字的區別](#cc中static關鍵字的區別)
	- [static（C）](#staticc)
	- [static（C++）](#staticc)
...
## static（C）
- C語言中，`static`用在**函數/全局變量**前，
表示該**函數/全局變量**爲當前文件的**內部函數/全局變量**，
只能在該文件中被訪問，當前文件被其它源碼include時該變量**不可訪問**。
- static用在局部變量前，則成爲**靜態**局部變量，
靜態局部變量**不在**棧區分配內存而是在**全局變量區**中分配空間，
因此保存在靜態局部變量中的值不會隨着函數運行結束而被**自動釋放**，下次執行該函數，
該函數內部的靜態局部變量不會重新分配空間，因而而是依然使用上次函數執行結束時的值。

## static（C++）
在C++中，除了C語言中的用法之外，還可以用在類的成員前，
