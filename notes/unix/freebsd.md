...
<!-- 多功能用戶管理工具 -->
# pw ...
# pw group mod 用戶組 -m 用戶名 <!-- 向指定用戶組添加用戶 -->
# pw group mod 用戶組 -d 用戶名 <!-- 從指定用戶組刪除用戶 -->
```

## sudo權限
FreeBSD系統默認不包含sudo工具，需要使用包管理器安裝：

```
# pkg install sudo
```

安裝sudo后，配置位於`/usr/local/etc/sudoers`，可注釋掉`wheel`或`sudo`相關段落。

為普通用戶添加sudo權限可將用戶加入wheel用戶組：

```
# pw group mod wheel -m 用戶名
```
