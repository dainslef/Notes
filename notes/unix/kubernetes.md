...
Kubernetes可能並未提供與操作系統版本匹配的倉庫，但由於Kubernetes使用Golang實現，
因而對操作系統依賴較少，在版本較新的操作系統中使用舊版本的Kubernetes倉庫通常不會有問題。
...
除了Kubernetes軟件包自身，還需要安裝匹配的容器運行時，如Docker，
在
...
以及`[plugins."io.containerd.grpc.v1.cri".registry.mirrors]`部分（該配置項默認已創建，但默認爲空）：
...
初始化集羣時可設定各類參數，同樣需要設置阿裏鏡像源，否則無法完成初始化：
