...
```html
$ find 路徑 -mtime 天數 <!-- 查找指定天數內未修改的文件 -->
```
...
使用`-delete`參數刪除查找到的內容：

```html
$ find 路徑 -name "*關鍵字*" -delete
```
...
```html
<!--
默認行爲：將源路徑下的文件同步到目標路徑（不包含子目錄）。
同步文件時僅同步文件内容，不包含文件用戶、權限、修改時間等其它信息。
-->
$ rsync 源路徑 目標路徑

<!--
將源路徑目標路徑下，在目標路徑下創建源路徑目錄的同名目錄。
同步文件時與默認模式相同，僅同步文件内容，不包含文件用戶、權限、修改時間等其它信息。
-->
$ rsync -r 源路徑 目標路徑

<!-- 將源路徑下的所有內容同步到目標路徑下，而非創建源路徑的同名目錄 -->
$ rsync -r 源路徑/ 目標路徑

<!-- 類似于 -r 參數用法，但同步文件時保留全部信息，包括文件用戶、權限、修改時間等其它信息 -->
$ rsync -a 源路徑 目標路徑
$ rsync -a 源路徑/ 目標路徑
```

rsync支持增量同步，默認會使用一套快速檢查算法，
通過比較文件大小、修改時間確定哪些文件需要同步；
...
- 使用`--size-only`參數調整比較算法，僅比較大小，忽略文件時間：

    ```html
    $ rsync -Pr --size-only 源路徑 目標路徑
    ```

rsync支持通過SSH進行文件同步：

```html
<!-- SSH跨主機複製可能面臨不同主機的賬戶差異，使用 -r 而非 -a 更爲通用 -->
...
| 參數 | 說明 |
| :- | :- |
| -c / --checksum | 使用基於checksum的文件校驗 |
| -z / --compress | 傳輸數據時使用壓縮算法，壓縮算法對機器資源消耗較大，僅推薦在配置較高的機器中使用（本地傳輸不建議使用） |
| -r / --recursive | 遞歸同步目錄下的所有内容，但僅同步文件内容，不 |
...
<!-- 查看當前時間 -->
$ date
Sun Jun  4 06:04:00 AM CST 1989

<!-- 輸出時間到Unix時間戳 -->
$ date +%s <!-- 精確到秒 -->
$ date +%s%N <!-- 精確到毫秒 -->
$ date +%s%3N <!-- 精確到微秒 -->
$ date -d '時間文本' +%s <!-- 將指定時間文本轉換為Unix時間戳 -->
$ date -d 'Jun 4 6:4:0 1989' +%s
612914640
...
```
# tcpdump -Xvni any 協議 port xxx or xxx
```
...
```html
<!-- 將抓包內容保存到pcap文件中 -->
# tcpdump -w pcap文件 其它參數...

<!-- 讀取pcap文件中保存的數據包信息 -->
# tcpdump -r pcap文件 其它參數...
```
...
docker與nerdctl容器對外掛文件系統的實現機制略有不同，
若在宿主機的容器綁定路徑上掛載新分區，二者行為存在差異：

- 使用nerdctl創建的容器，掛載新分區時容器可立即感知；
- 使用docker創建的容器，掛載新分區時容器無變化，需要重啟容器方可讀取新掛載的內容。
...
<!-- 清理無用依賴 -->
# apt-get autoremove
# apt-get autoremove --purge <!-- 清理無用依賴同時移除配置 -->
# apt-get autopurge <!-- 等價 autoremove --purge -->
