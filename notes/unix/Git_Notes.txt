...
关于Git的配置 >
使用git config指令会将配置写入当前仓库的.git/config路径下，只对当前仓库生效。
使用git config --global指令会将配置写入全局路径下(在Linux/macOS/BSD中是家目录下的.gitconfig文件)。
全局配置的优先级低于当前仓库的配置，当前仓库的配置与全局配置冲突时会覆盖全局配置。
...
$ git pull [远程仓库名] :[远程分支名称]
在Git1.7之后，可以以下指令删除远程分支，作用相同：
$ git pull [远程仓库名] --delete [远程分支名称]

----

强制覆盖远程分支的内容 >
强制覆盖分支的内容需要修改远程仓库中的receive配置：
$ git config receive.denyCurrentBranch ignore
同时，本机在执行push操作时使用--force参数：
$ git push [远程仓库名] [同名分支] --force
$ git push [远程仓库名] [本地分支名称]:[远程分支名称] --force
如果没有使用--force参数，分支内容合并有冲突时依旧会拒绝合并而不是强制覆盖。
