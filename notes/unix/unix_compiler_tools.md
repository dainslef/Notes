...
gcc/clang有`O0`、`O1`、`O2`、`O3`三個代碼優化級別；
`O0`最低（默認選項，不啟用任何優化），`O3`優化最高。

使用優化選項能大幅度提升生成二進制文件的執行效率，但會使得生成的程序難以調試；
故一般只在程序完成測試之後進入發佈階段才會啟用編譯優化選項。

通常推薦使用O2模式，能夠平衡性能與生成二進制文件的大小；
使用O3模式會最大化優化性能，但會降低編譯速度，提升生成二進制文件的大小；
部分程序/庫在使用O3模式編譯時會產生錯誤。
...
# Make
在Unix环境下通常使用make工具来构建项目/工程。

make工具通過解析項目目錄下的makefie文件執行編譯流程：

```html
$ make <!-- 默認讀取當前目錄下的makefile文件 -->
$ make -f 指定makefile文件

$ make -d/--debug <!-- 打印調試信息 -->
$ make --debug=a/b/v <!-- 按指定級別打印調試信息，a=all（默認，與-d參數相同），b=basic（基礎調試信息），v=verbose（最詳細） -->
```
