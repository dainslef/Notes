...
> **换行符选项**取值含义：
>
>	- `true` 在`Windows`提交到`Unix`环境时使用，提交时将`CRLF`转换为`LF`，而签出代码时自动将`LF`转换为`CRLF`
>	- `input` 在`Unix`环境中使用，提交时`CRLF`换行符转换为`LF`，而签出代码时`LF`不转换为`CRLF`
>	- `false` 在`Windows`环境中使用，不转换`CRLF`，直接提交

### 配置文件
`Git`配置文件分为**全局配置文件**和**仓库配置文件**。

- 仓库配置文件路径为`[仓库路径]/.git/config`，使用`git config`指令会将配置写入此文件。
- 在`Linux/macOS/*BSD`中，全局配置文件路径为`~/.gitconfig`，使用`git config --global`指令会将配置写入此文件中，
...
- `$ git log` 交互式查看版本库的历史提交记录和`commit_id`
- `$ git log --pretty=oneline` 仅仅查看提交的`commit_id`

可以根据每次提交的`commit_id`回滚到历史版本：
...
### 推送本地仓库
添加完密钥之后，查看已创建的仓库的SSH地址，将此地址添加到远程仓库中：
...
若`GitHub`上创建的仓库非空(比如创建仓库的同时加入了许可证文件)，则可能推送失败(文件冲突)，此时可以先将本地版本库拉到`GitHub`上对应分支的版本：
...
1. `git reflog expire --expire=now --all` 清除分支变更历史
1. `git gc --prune=now` 压缩到目前为止的历史提交信息
