...
## 修改历史提交
要修改历史版本的提交，首先要重构目标提交所在的提交的前一次提交。

修改历史提交，需要执行以下步骤：

1. 执行`git rebase -i [commit_id]`，之后会进入交互式的历史提交查看页面，
在此页面将要修改的提交说明前的`pack`字段改为`edit`(若仅需修改历史提交说明，则修改为`reword`)，
可设置多个需要编辑的提交。
1. 字段修改完毕后，Git会自动支跳转到该次提交，编辑该次提交(reword模式下，会直接跳转到提交说明编辑页面)。
1. 之后重新构建提交，执行`git rebase --continue`，Git会跳转到下一个编辑的提交，
重复以上操作，直至跳转到最近的提交。
...
当本地分支内容与远程分支发生冲突时，可以强制更新远程分支到本地：
