...
- `Suspend`中译**睡眠**，睡眠状态下，系统将关闭大部分硬件的工作，系统状态将会被保存在`RAM`中。处于睡眠状态下的计算机将以极低的功耗运转，但系统仍需要供电，电源指示灯会依旧闪烁。现代计算机**进入睡眠状态/从睡眠状态唤醒**仅仅需要几秒时间。
- `Hibernate`中译**休眠**，休眠即`Suspend to disk`，系统将完全关闭所有硬件，系统状态将被保存在`SWAP`中，处于休眠状态下的计算机不需要供电，但系统分区时必须分配并挂载了`SWAP`交换区。**休眠**从硬盘中加载系统状态，对于使用`HDD`的计算机，**进入休眠状态/从休眠状态唤醒**需要几十秒甚至更久，相当于正常开机的速度。
