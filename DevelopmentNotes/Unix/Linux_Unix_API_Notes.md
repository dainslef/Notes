...
参数是一个`time_t`类型变量的地址，函数会返回当前系统的时间，同时将当前系统的时间写入传入的地址中。
...
读取的内容的首地址存放在`buf`指针中，`count`为设定读取的字节数，返回值为实际读取了的字节数。
失败时返回`-1`。
...
`buf`为要写入内容的首地址，`count`指定写入数据的长度，`buf`指向的内容中超过`count`长度的内容**不会**被写入。
返回值为**实际写入内容的大小**。
...
其中，`sig`为要处理的信号，`func()`为要绑定的信号处理函数，这是传统Unix的信号处理方式。
...
`sem_ops`参数为指向`sembuf`结构体的指针(有多个信号量时可以指向`sembuf`结构体数组)，`sembuf`结构体的定义为：
